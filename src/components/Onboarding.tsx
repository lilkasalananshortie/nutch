...
export function Onboarding({ onComplete }: { onComplete: () => void }) {
  ...
  return <section className="onboarding-panel" aria-labelledby="onboarding-title">
    ...
    {step === 2 && <><h1 id="onboarding-title">How should it open?</h1><p>Choose the interaction that feels natural to you.</p><div className="onboarding-options"><button className={settings.hoverToExpand ? "selected" : ""} onClick={() => { if (!settings.hoverToExpand || settings.clickToExpand) void update({ hoverToExpand: !settings.hoverToExpand }); }}><strong>Hover to expand</strong><small>Reveal controls when your pointer enters</small></button><button className={settings.clickToExpand ? "selected" : ""} onClick={() => { if (!settings.clickToExpand || settings.hoverToExpand) void update({ clickToExpand: !settings.clickToExpand }); }}><strong>Click to expand</strong><small>Open the island intentionally</small></button></div><button className="primary-button" onClick={() => setStep(3)}>Continue</button></>}
